...
let TenDeliveriesOutput = { R: 6, S:6,T:12,U:12,V:18,W:6,X:6,Y:12,Z:12,A:18 };
let DistributedDeliveryOutput = { R: 18,T:12,W:6,X:6,Y:12,Z:12,A:18 };
...
describe("Extra Runs conceded per team", () => {
  test("Check for five matches", () => {
    expect(functions(TenMatches, TenDeliveries)).toStrictEqual(TenDeliveriesOutput);
  });
  test("Check for five matches", () => {
    expect(functions(DistributedMatch, DistributedDelivery)).toStrictEqual(DistributedDeliveryOutput);
  });
